...
const [employerCount, setEmployerCount] = useState(0);
  ...
  useEffect(() => {
    async function loadDashboard() {
      ...
      setEmployerCount(overview.totalEmployers);
      const jobsResult = await adminJobsApi.getAll();
     if (jobsResult.ok) {
        const sorted = [...jobsResult.jobs]
...
if (usersResult.ok) {
  const sortedUsers = [...usersResult.users]
      ...
      <div className="grid grid-cols-1 gap-6 lg:grid-cols-3">
        ...
        <div className="rounded-2xl border border-gray-100 bg-white p-4 transition-shadow duration-200 hover:shadow-md sm:p-6 lg:col-span-2">
          <div className="mb-4 flex items-center justify-between">
            <h2 className="text-sm font-bold text-gray-900 sm:text-base">Recent job activity</h2>
          </div>
          <div className="flex flex-col gap-2">
            {recentJobs.map((job) => (
              <div key={job.id} className="flex items-center justify-between rounded-xl bg-gray-50 px-4 py-3 transition-colors hover:bg-[#EDE7F8]">
                <div>
                  <p className="text-sm font-semibold text-gray-900">{job.title}</p>
                  <p className="mt-0.5 text-xs text-gray-500">{job.employer?.companyName ?? "Unknown company"}</p>
                </div>
            ...
            {recentJobs.length === 0 && (
              <p className="py-6 text-center text-sm text-gray-400">No jobs posted yet.</p>
            )}
          </div>
        </div>
...
<div className="rounded-2xl border border-gray-100 bg-white p-4 transition-shadow duration-200 hover:shadow-md sm:p-6">
 ...
 <div className="mt-4 flex flex-col gap-2.5 border-t border-gray-100 pt-3 text-xs sm:text-sm">
  <div className="flex justify-between">
    <span className="text-gray-500">Registered employers</span>
    <span className="font-semibold text-gray-900">{employerCount}</span>
  </div>
  <div className="flex justify-between">
    <span className="text-gray-500">Active job postings</span>
    <span className="font-semibold text-gray-900">{activeJobCount}</span>
  </div>
</div>
</div>
      </div>
  ...
  <p className="text-[11px] font-semibold tracking-wide text-[#8A38F5] uppercase sm:text-xs">Talent Pool</p>
  <p className="mt-2 text-xs font-bold text-gray-900 sm:text-sm">{candidateCount} registered candidates</p>
  <p className="mt-1 text-xs text-gray-500 sm:text-sm">Review and manage candidate accounts.</p>
  <Link
    href="/admin/users"
    className="mt-4 inline-block rounded-xl bg-gray-100 px-3 py-1.5 text-xs font-semibold text-gray-900 transition-colors hover:bg-[#8A38F5] hover:text-white sm:px-4 sm:py-2 sm:text-sm"
  >
    View profiles
  </Link>
</div>
  ...
  {pendingVerifications.length > 0 ? (
    ...
    <p className="mt-1 text-xs text-gray-500 sm:text-sm">
     ...
     {/* Latest updates */}
<div className="rounded-2xl border border-gray-100 bg-white p-4 transition-shadow duration-200 hover:shadow-md sm:p-6">
  <h2 className="mb-4 text-sm font-bold text-gray-900 sm:text-base">Recently joined</h2>
  <div className="flex flex-col gap-3">
    {recentUsers.map((user) => (
      <div key={user.id} className="flex items-center justify-between">
        <div className="flex items-center gap-2.5">
          <span className="flex h-7 w-7 shrink-0 items-center justify-center rounded-full bg-[#EDE7F8] text-[10px] font-semibold text-[#8A38F5]">
        ...
        <span className="shrink-0 text-xs text-gray-400">{formatTimeAgo(user.createdAt)}</span>
      </div>
    ))}
